...
在服务端用战局会话持有独立于传输连接的玩家实体，处理加入与离开，并通过窄事件与发送门面与网络交互，为后续同步留下稳定边界。

## Requirements

### Requirement: Player identity is not connection identity
战局 SHALL 为每名已加入玩家分配独立于传输连接标识的玩家标识。同一连接 MUST 在一局内至多对应一名玩家。玩家标识 MUST NOT 被规定为等于连接标识。

#### Scenario: Join allocates player id
- **WHEN** 战局接受某连接的加入且该连接尚无玩家
- **THEN** 战局 MUST 存在一名该连接对应的玩家，且其玩家标识与连接标识允许不同

#### Scenario: Duplicate join rejected
- **WHEN** 已加入的连接再次请求加入同一战局
- **THEN** 系统 MUST 拒绝第二次加入，且 MUST NOT 再分配另一名玩家

### Requirement: Leave is notified by battle not transport
连接被传输层彻底移除（含宽限期结束后移除）时，系统 SHALL 将其翻译为战局玩家离开并通知战局。战局处理离开时 MUST NOT 读取传输服务端的连接表或会话绑定。未加入战局的连接被移除时 MUST NOT 要求战局移除玩家。

#### Scenario: Removed connection leaves player
- **WHEN** 已加入战局的连接被传输层移除
- **THEN** 战局 MUST 移除对应玩家，且该路径 MUST 不查询传输服务端子系统
