...
- 阶段 3 已完成：`NetworkInterpolationCapability` 持有 `SnapshotInterpolator` 和最近输入快照；Replica 按服务端 Tick 缓冲完整 Transform 快照，Position 使用 `Vector3.Lerp`，Rotation 使用最短路径 `Quaternion.Slerp`。首个快照以及超过 `positionSnapThreshold` 或 `rotationSnapThresholdDegrees` 的传送会清空旧缓冲并立即 Snap，缓冲不足时保持最近显示状态。
- 阶段 4 已完成：`NetworkPredictionCapability` 持有预测历史、确认 Tick 与重放帧；Predict 实体每个网络 Tick 通过 `NetworkInputCapability` 采样输入，通过 `NetworkSimulationCapability` 立即模拟。服务端确认后按 Position/Rotation 阈值回滚重放；普通纠错由 `NetworkTransformCapability` 保持表现节点世界姿态并按半衰期平滑收敛，超过 Snap 阈值时直接校正。
- 阶段 5 已完成：删除 `NetworkSync/Compatibility` 下的旧 Role、Controller、Position Snapshot/Sync 和旧同步接口；`GameScene.unity` 不再保存 Prefab 实例级旧身份补丁，`NetPlayer.prefab` 成为能力组合的唯一装配来源；`AGENTS.md` 已更新为 `EntitySystem/`、`EntitySimulationCore/` 与 `NetworkSync/` 的当前目录边界。
- 阶段 6 已完成：`NetworkObjectIdentity` 不再依赖或创建 `EntityCharacter`、`PlayerController`，只维护身份、能力生命周期与复制注册；`NetworkObjectComponentActivator` 建立 Capability 与 Channel 索引，拒绝重复能力、校验依赖并保证激活幂等。Transform、InputCommand、Simulation、Snapshot、Prediction、Interpolation 六类能力分别持有实际操作或状态，`EntityReplicationSystem` 只编排已激活能力；`TestSimulator` 只实例化已经在 Prefab 中装配好的对象。
...
- 后续工作：输入批次与每连接速率限制、网络实体工厂与对象池的完整生命周期测试。
