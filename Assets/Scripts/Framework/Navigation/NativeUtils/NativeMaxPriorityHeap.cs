...
namespace Framework
{
    public struct NativeMaxPriorityHeap<T> : IDisposable where T : unmanaged
    {
        ...
        public NativeMaxPriorityHeap(int startCapacity, Allocator allocator)
        {
            if (startCapacity < 0)
            {
        ...
        public void Push(T key, float value)
        {
            ...
            PushUnsafe(key, value);
        }

        /// <summary>
        /// Fixed-capacity insert that keeps the smallest values and drops larger ones.
        /// </summary>
        public void TryPushSmallest(T key, float value)
        {
            if (Count == _capacity)
            {
                if (Count > 0 && PeekValue > value)
                {
                    _keys[1] = key;
                    _values[1] = value;
                    BubbleDown(1);
        ...
        private void PushUnsafe(T key, float value)
        {
            Count++;
            _keys[Count] = key;
            _values[Count] = value;
