...
namespace GamePlay.EntitySystem
{
        ...
        INetSyncSnapshotReceiver<NetPositionSnapshot>
    {
        ...
        /// 绑定同步根对象。
        /// </summary>
        public void Bind(NetworkObjectIdentity syncRoot)
        {
            _syncRoot = syncRoot;
        }
        ...
        /// 根据网络角色配置同步行为。
        /// </summary>
        public void ConfigureRole(EntitySimulationMode role)
        {
            if (_movementModule)
            {
                _movementModule.SetReplicaMode(role == EntitySimulationMode.Replica);
            }

            if (role == EntitySimulationMode.Replica)
            {
