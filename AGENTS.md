...
6. **Methods** — group by feature under feature-named `#region` blocks (e.g. `#region 实体控制`, `#region 控制器方法`, `#region 模拟入口`, `#region 状态属性`). Public API first, then private helpers.
...
Reference implementations: [NetEntityIdentity.cs](file:///d:/Project/Unity_Project/Skuy's%20Framework/Assets/Scripts/GamePlay/MultiPlaySystem/Component/NetEntityIdentity.cs) (serialized fields → `#region 属性` → `#region 事件` → methods) and [EntityCharacter.cs](file:///d:/Project/Unity_Project/Skuy's%20Framework/Assets/Scripts/GamePlay/EntitySystem/Character/EntityCharacter.cs) (fields → `#region 属性` → feature regions → `#region 生命周期` at end).

### File organization
- One primary type per file; file name matches the type.
- Apply the member order above; do not interleave fields, properties, and methods out of order. `#region` labels use short Chinese text consistent with the module — `#region 属性`, `#region 事件`, `#region 生命周期`, `#region 控制器方法`, `#region 状态属性`, `#region 模拟入口`. Mirror the regions already used in the file you are editing.
